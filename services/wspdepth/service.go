...
type Server struct {
	pb.UnimplementedPartialDepthEventServiceServer
	Db *mongo.Database
...
func Register(grpc *grpc.Server, s *Server) {
	pb.RegisterPartialDepthEventServiceServer(grpc, s)
}
...
func toDoc(event *pb.WsPartialDepthEvent) *model.WsPartialDepthEvent {
	...
	return &model.WsPartialDepthEvent{
		Symbol:       event.Symbol,
